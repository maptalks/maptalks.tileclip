export default ` (function (exports) { 'use strict';
  ...
  const abortControlCache = {};
  function cacheFetch(taskId, control) {
      abortControlCache[taskId] = abortControlCache[taskId] || [];
      abortControlCache[taskId].push(control);
  }
  function cancelFetch(taskId) {
      const controlList = abortControlCache[taskId] || [];
      if (controlList.length) {
          controlList.forEach(control => {
              control.abort(new Error('fetch tile data cancel'));
          });
      }
      delete abortControlCache[taskId];
  }
  function finishFetch(control) {
      const deletekeys = [];
      for (let key in abortControlCache) {
          const controlList = abortControlCache[key] || [];
          if (controlList.length) {
